...
int main(int argc, char *argv[])
{
    ...
    result = PQexec(conn, "SAVEPOINT mypoint");
    ASSERT_TRUE(PGRES_COMMAND_OK == PQresultStatus(result));
    printf("transaction save point success\n");
   
    num = insertData(conn, "INSERT INTO libpqtable VALUES (2)");
    ASSERT_TRUE(num == 1);
    printf("insert data into table success\n");
   
    result = PQexec(conn, "ROLLBACK TO mypoint");
    ASSERT_TRUE(PGRES_COMMAND_OK == PQresultStatus(result));
    printf("transaction rollback to save point success\n");
