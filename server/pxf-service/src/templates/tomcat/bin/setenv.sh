...
PXF_OPTS="-Dpxf.home=${PXF_HOME} -Dpxf.conf=${PXF_CONF} -Dconnector.http.port=${PXF_PORT} -Dbase.shutdown.port=${PXF_SHUTDOWN_PORT} -Dpxf.log.dir=${PXF_LOGDIR} -Dpxf.service.user.impersonation.enabled=${PXF_USER_IMPERSONATION} -Dpxf.service.kerberos.keytab=${PXF_KEYTAB} -Dpxf.service.kerberos.principal=${PXF_PRINCIPAL}"
