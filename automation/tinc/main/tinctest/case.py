...
from qautils.gppylib import gpversion
...
class TINCTestCase(unittest.TestCase):
    ...
    #: Deprecated: A list of existing JIRAs against which a test failure will do a dedup match
    test_jira_list = []
    ...
    def __init__(self, methodName, baseline_result = None):
        ...
        pass or fail for the current tun. Currently set only for tests run through tincdb.
        """
...
class _TINCProductVersionRange(object):
    ...
    def _parse_product_version_range(self):
        ...
        # If one of the versions is a hotfix version, fail
        if self.upper_bound_version.hotfix or self.lower_bound_version.hotfix:
            raise TINCInvalidProductVersionException("Hotfix versions should not provided within a range - %s" %self.version_range_str)
...
class _TINCProductVersion(object):
    ...
    def incr(self):
        ...
        """
        # Increment not supported for wildcard versions or hotfix versions
        if 'x' in self.version or self.hotfix:
    ...
    def decr(self):
        ...
        """
        # Decrement not supported for wildcard versions or hotfix versions
        if 'x' in self.version or self.hotfix:
            return _TINCProductVersion(self.version_str)
