...
# check if the DEB for GP version and TARGET_OS_VERSION is available
deb_file_name=$(find pxf_artifacts -type f -name "pxf-gp${GP_VER}-*-1-${TARGET_OS_VERSION}-amd64.deb" -printf "%f\n")
[[ -n ${deb_file_name} ]] || fail "pxf_artifacts/pxf-gp${GP_VER}-*-1-${TARGET_OS_VERSION}-amd64.deb not found"
...
mv rebuild.deb pxf-gp${GP_VER}-$(dpkg-deb --field rebuild.deb Version)-${TARGET_OS_VERSION}-amd64.deb

# check if the new DEB for GP version and TARGET_OS_VERSION is available
new_deb_file_name=$(find ./ -type f -name "pxf-gp${GP_VER}-*-2-${TARGET_OS_VERSION}-amd64.deb" -printf "%f\n")
[[ -n ${new_deb_file_name} ]] || fail "./pxf-gp${GP_VER}-*-2-${TARGET_OS_VERSION}-amd64.deb not found"
